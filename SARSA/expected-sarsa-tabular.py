...
if __name__ == "__main__":
    ...
    if loading:
        Q = np.load('./sarsa-save/Esarsa_Q_tensor.npy')
    else:
    ...
    for i_episode in range(10000):
        ...
        for t in range(500):
            ...
            next_maxAction = np.argmax(Q[next_pos_idx, next_vel_idx]) # The best action to take in next state
            # Probability of choosing non-max action is epsilon / numActions
            nonMaxProb = epsilon / action_size
            maxProb = 1 - epsilon + epsilon / action_size

            # Expected Value sum
            expected_sum = 0
            for next_action in range(action_size):
                if next_action == next_maxAction:
                    expected_sum += maxProb*Q[next_pos_idx, next_vel_idx, next_action]
                else:
                    expected_sum += nonMaxProb*Q[next_pos_idx, next_vel_idx, next_action]

            # Update Q
            Q[pos_idx, vel_idx, action] += alpha * (reward + gamma*expected_sum - Q[pos_idx, vel_idx, action])
        ...
        if saving and i_episode % 100 == 0:
            np.save('./sarsa-save/Esarsa_Q_tensor', Q)
            np.save('./sarsa-save/Esarsa_episode_rewards', np.array(episode_rewards))